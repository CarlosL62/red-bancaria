...
## Estado
Última actualización: 2026-09-09 (config v8.6 persistida: **política SSH refinada host-based "solo PCs admin"** — ACLs por host exacto + password de gestión rotado + cliente SSH verificado en ambos PCs admin + configuración persistida en config-disk master `IOSv_startup_config.img` v8.6 y validada con rearranque real del nodo). **Cajas de caja Y PC admin-sucursal operativos con Firefox y usuario OS `admin` (sede `.10.4`, sucursal `.40.3`, admin-sucursal `.50.3`)** — re-imagen de la sucursal a TC6+Firefox, endurecimiento de la imagen sede (identidad autocontenida) y admin-sucursal re-imagenado con la misma base (nodo a `ide`/RAM 512). Sin cambios de red (NAT/rutas/tracks/servicios intactos).
...
* **Cliente SSH en los PCs admin (verificado, SIN inyección):**
  ...
  * admin-sucursal: **re-imagenada 2026-09-09 a la base Firefox TC6** (`linux-tinycore-admin-suc-firefox.img`), que incluye `openssh.tcz`; `bootlocal.sh` arranca sshd con IP `172.16.50.3/24` y gw `172.16.50.1` → siguen sshd y cliente SSH, y ahora también Firefox.
...
## Cajas de caja y PC admin-sucursal (OS con Firefox, usuario `admin`) — 2026-09-09
* **caja-sede `172.16.10.4`**, **caja-sucursal `172.16.40.3`** y **PC admin-sucursal `172.16.50.3`** idénticos: TinyCore con **Firefox**, usuario OS `admin`/4170 (uid 1002), sshd en cada boot, IP/gw propios aplicados por `bootlocal.sh`. Cajas: getty serial `ttyS0` (`gns3`/`gns3`); admin-suc: consola VNC.
* Imágenes registradas en GNS3 (md5 real en sidecar `.md5sum`):
  ...
  * `linux-tinycore-admin-suc-firefox.img` — md5 `492fe230e88465e24a672a7ba44e202b`. Aplica la misma base Firefox al PC admin-sucursal (antes `linux-tinycore-11.1.qcow2`, sin navegador): IP `172.16.50.3` **mask `/24`** (red `172.16.50.0/24`), gw `172.16.50.1`. El nodo también pasó de hda **`virtio` a `ide`** (el base TC6 no porta virtio) y RAM 200→**512**.
* Nodos: caja-sede (`e5a409b1`) hda **`ide`**, RAM 256; caja-sucursal (`ad8bb62d`) hda **`ide`**, RAM **512**; admin-sucursal (`5ba64674`) hda **`ide`**, RAM **512**, consola **VNC 5901**.
...
* **Verificación 2026-09-09 (real, desde admin-sede):** ping OK a `.10.4`, `.40.3` y `.50.3`; `ssh admin@` con 4170 → `uid=1002(admin)` e IP/mask correctas en los tres (`.40.3`/`255.255.255.240`, `.50.3`/`255.255.255.0`). Serial de la caja-suc muestra login (`Core Linux ... box login`). Portal `http://172.16.30.5` responde HTML "Banco 1 - Portal Interno": desde la red y **desde `.50.3` (wget real por SSH)** → admin-sucursal con Firefox puede abrirlo por VNC `:5901` y la caja-suc por VNC `:5905` (confirmación visual pendiente por el usuario).
...
* 2026-09-09: **PC admin-sucursal `.50.3` re-imagenado a base Firefox (requiere aprobación previa, sin tocar red).** Nueva imagen `linux-tinycore-admin-suc-firefox.img` (md5 `492fe230e88465e24a672a7ba44e202b`) sobre la misma base TC6+Firefox de las cajas: IP `172.16.50.3/24`, gw `172.16.50.1`, identidad `admin`/4170 uid 1002 + `gns3`+`tc`, sshd en cada boot. Nodo `5ba64674` actualizado vía API: hda `ide` (antes virtio; el base TC6 no porta virtio) y RAM 512. **Validado 2026-09-09:** ping `.50.3` OK desde admin-sede; `ssh admin@172.16.50.3` 4170 → `uid=1002(admin)`, `inet addr:172.16.50.3  Mask:255.255.255.0`; `wget http://172.16.30.5` desde `.50.3` → HTML "Banco 1 - Portal Interno" real. Consola VNC 5901 (screendump QEMU con contenido 1024x768 → escritorio con Firefox; confirmación visual pendiente del usuario).
