...
* **Estado actual (PENDIENTE DE CONFIRMACIÓN POR BANCO 2):** el camino B1 **oeste** (fuente `.2`) quedó verificado sano de extremo a extremo; si B2 está usando fuente `.5`/arco este (vía B5, que B1 ve flapeando ICMP), el tráfico entra por Gi0/2 donde **no hay traducción persistente** para su destino `10.0.0.1:8080`. Acción solicitada a B2: reintentar con `curl -v --connect-timeout 8 http://10.0.0.1:8080/api/blacklist` desde la cara oeste mientras B1 observa contadores y tcpdump en el FW (permitió confirmar en una sola pasada). Si insiste por el este, evaluar con B1/B2 publicar también el estático en Gi0/2 (segundo estático `10.0.0.18`, sujeto a la limitación IOSv documentada).
