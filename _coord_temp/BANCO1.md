...
## Estado
Última actualización: 2026-09-08 (config v8.4 E2E con Local PBR + endpoint `/interbancaria` + apartado "Transferencia a Banco 2" en el portal)
...
* **`10.0.0.18:8080` → `172.16.30.6:8080`** (cara este, agregado 2026-09-08 ~09:51): nuevo `ip nat inside source static tcp 172.16.30.6 8080 interface GigabitEthernet0/2 8080`, aplicado y **persistido** (`write memory` OK, confirmado en running-config). Antes `10.0.0.18:8080` daba `Could not connect` (rechazo local del router, sin estático). Publica la blacklist también por la entrada este (`http://10.0.0.18:8080/api/blacklist`) para B5/B4. **PENDIENTE DE CONFIRMACIÓN POR BANCO 5/4:** validación E2E real desde su nodo (el retorno sale por default `via 10.10.2.1`; FW sin ruta fantasma `10.0.0.16/30` desde el fix de persistencia previo).
...
* IOSv: el estático temporal con global `10.0.0.18` no persistía en pruebas previas; tras el `write memory` de 2026-09-08 ~09:51 el estático `172.16.30.6:8080 → Gi0/2:8080` sí quedó en running-config/NVRAM.
...
* 2026-09-08 ~09:51: **blacklist publicada también por la cara este (`10.0.0.18:8080`)** en `Router-salida` (consola GNS3): `ip nat inside source static tcp 172.16.30.6 8080 interface GigabitEthernet0/2 8080`, `write memory` OK, confirmado en running-config (`show ip nat translations`: `tcp 10.0.0.18:8080 → 172.16.30.6:8080`). Antes fallaba con `Could not connect ... 117 ms` (sin estático). PENDIENTE DE CONFIRMACIÓN POR BANCO 5/4 (validación E2E real).
